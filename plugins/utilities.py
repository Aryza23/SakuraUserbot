...
try:
    ...
    telegraph.create_account(short_name="Sakura")
...
async def leave(ult):
    if not ult.out and not is_fullsudo(e.sender_id):
        return await eod(ult, "`perintah ini dibatasi untuk nggota sudo.`")
    await eor(ult, f"`{ultroid_bot.me.first_name} telah meninggalkam group, bye!!.`")
