...
async def uconverter(event):
    ...
    if not (a.media and a.media.document and a.media.document.mime_type in ok):
        return await eor(event, "`balas ke sticker...`")
...
async def pack_kangish(_):
    ...
    if not _e:
        return await eor(_, "`balas ke sticker.`")
    if len(_.text) > 9:
        _packname = _.text.split(" ", maxsplit=1)[1]
    else:
        _packname = f"Sakura kang pack by {_.sender_id}"
    if _e and _e.media and _e.media.document.mime_type == "image/webp":
        ...
        await eor(
            _,
            f"✨ ditambahkan ke dalam pack.\nlihat pack sticker: [link](https://t.me/addstickers/{_r_e_s.set.short_name})",
        )
    else:
        await eor(_, "jenis file tidak didukung.")
...
async def hehe(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            await xx.edit("`jenis file tidak didukung!`")
            return
    else:
        await xx.edit("`gagal mencuri sticker...`")
        return
    ...
    if photo:
        ...
        if (
            ...
            async with ultroid_bot.conversation("@Stickers") as conv:
                ...
                while "120" in x.text:
                    ...
                    if x.text == "Invalid pack selected.":
                        ...
                        await ultroid_bot.send_read_acknowledge(conv.chat_id)
                        await xx.edit(
                            f"`stiker ditambahkan ke pack yang berbeda !\
                            \npack baru telah dibuat!\
                            \nlihat pack nya` [disini](t.me/addstickers/{packname})",
                            parse_mode="md",
                        )
                        return
                if is_anim:
                ...
                if "Sorry, the file type is invalid." in rsp.text:
                    await xx.edit(
                        "`gagal menambahkan sticker, gunakan bot` @Stickers `dan tambahkan secara manual.`",
                    )
                    return
                await conv.send_message(emoji)
                await ultroid_bot.send_read_acknowledge(conv.chat_id)
            ...
            await xx.edit("`membuat pack baru...`")
            async with ultroid_bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                await ultroid_bot.send_read_acknowledge(conv.chat_id)
...
async def ultdround(event):
    ...
    if not (ureply and (ureply.media)):
        await xx.edit("`balas ke media`")
        return
    ultt = await ureply.download_media()
    if ultt.endswith(".tgs"):
        await xx.edit("`memproses sticker...`")
...
async def ultdestroy(event):
    ...
    if not event.is_reply:
        return await eor(event, "`balas ke sticker animasi saja...`")
    if not (
